...
/**
 * إرسال التقرير للمراجعة وتحديد المراجع بالبريد الإلكتروني
 */
export async function submitForReview(
  ...
  if (email) {
    ...
    data.reviewRequestedAt = serverTimestamp();
  }

  await updateDoc(doc(db, "reports", reportId), data);

  await addHistoryEntry(reportId, {
    action: "submit",
...
export async function approveReport(reportId, profile, note = "") {
  ...
  if (!nextStatus) {
    throw new Error("هذا الدور لا يملك صلاحية الاعتماد");
  }

  await updateDoc(doc(db, "reports", reportId), {
    status: nextStatus,
    approvedBy: profile.name || profile.email,
    approvedByEmail: profile.email || "",
...
export async function returnForEdit(reportId, profile, note = "") {
  await updateDoc(doc(db, "reports", reportId), {
    ...
    returnedBy: profile.name || profile.email,
...
export async function rejectReport(reportId, profile, note = "") {
  await updateDoc(doc(db, "reports", reportId), {
    ...
    rejectedBy: profile.name || profile.email,
    rejectedByEmail: profile.email || "",
...
/**
 * جلب التقارير المرسلة إلى مراجع محدد بالبريد الإلكتروني
 */
export async function listReportsForReviewer(reviewerEmail) {
